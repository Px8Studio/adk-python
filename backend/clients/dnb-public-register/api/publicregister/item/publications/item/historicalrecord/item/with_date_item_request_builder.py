...
class WithDateItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /api/publicregister/{languageCode-id}/Publications/{registerCode}/historicalrecord/{date}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/api/publicregister/{languageCode%2Did}/Publications/{registerCode}/historicalrecord/{date}{?page*,pageSize*}", path_parameters)
    
    async def get(self,request_configuration: Optional[RequestConfiguration[WithDateItemRequestBuilderGetQueryParameters]] = None) -> Optional[PublicationView]:
        """
        Gets the publication which was published on the specified date for the specified language code and register code. Publicationsolder than one year can not be provided.
