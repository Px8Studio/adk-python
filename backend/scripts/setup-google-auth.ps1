...
try {
    ...
    if (-not (Test-Path $pythonExe)) {
        Write-Warning "Python virtual environment not found at: $pythonExe"
        Write-Info "Skipping Python verification"
    } else {
        Write-Info "Testing BigQuery authentication..."
        $bqTest = "from google.cloud import bigquery; client = bigquery.Client(); print('BigQuery authentication successful')"
        & $pythonExe -c $bqTest
        
        Write-Info "Testing credentials type..."
        $credTest = "import google.auth; creds, project = google.auth.default(); print(f'Authenticated as: {creds.service_account_email if hasattr(creds, ""service_account_email"") else ""user""}'); print(f'Project: {project}')"
        & $pythonExe -c $credTest
        
        Write-Success "Authentication verification complete!"
    }
} catch {
    Write-Warning "Could not verify Python authentication: $($_.Exception.Message)"
    Write-Info "You can manually test with: .venv\Scripts\python.exe -c ""from google.cloud import bigquery; client = bigquery.Client()"""
}
...
Write-Host "║  • NEVER commit service-account-key.json to git               ║" -ForegroundColor Yellow
Write-Host "║  • Already added to .gitignore                                 ║" -ForegroundColor Yellow
Write-Host "║  • Rotate keys every 90 days (set calendar reminder)          ║" -ForegroundColor Yellow
Write-Host "║  • Keep key file secure with restricted file permissions      ║" -ForegroundColor Yellow
Write-Host "╚════════════════════════════════════════════════════════════════╝`n" -ForegroundColor Yellow

Write-Success "Setup complete! You can now run your BigQuery uploads and ADK agents without re-authentication."
Write-Info "Documentation: See GOOGLE_CLOUD_AUTH_SETUP.md for more details"
